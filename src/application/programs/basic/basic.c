...
int basic(const char *script, FILE *in, FILE *out, FILE *err)
{
  ...
  while(curline != -1)
  {
    global->string = global->lines[curline].str;
	global->token = gettoken(global->string);
	global->errorflag = 0;

	nextline = line();
	if(global->errorflag)
	{
      reporterror(global->lines[curline].no);
	  answer = 1;
	  break;
	}
	...
	else
    {
	  ...
	  {
		if(global->fperr)
	      fprintf(global->fperr, "line %d not found\n", nextline);
		answer = 1;
		break;
	  }
    }
...
/*
  Sets up all our globals, including the list of global->lines.
...
static int setup(const char *script)
{
  ...
  global->nlines = mystrcount(script, '\n');
  global->lines = malloc(global->nlines * sizeof(LINE));
  if(!global->lines)
  {
	if(global->fperr)
      fprintf(global->fperr, "Out of memory\n");
	return -1;
  }
  for(i=0;i<global->nlines;i++)
  {
	if(isdigit(*script))
	{
      global->lines[i].str = script;
	  global->lines[i].no = strtol(script, 0, 10);
	}
	else
	{
	  i--;
  ...
  {
	if(global->fperr)
	  fprintf(global->fperr, "Can't read program\n");
    free(global->lines);
	return -1;
  }

  for(i=1;i<global->nlines;i++)
	if(global->lines[i].no <= global->lines[i-1].no)
	{
	  if(global->fperr)
		fprintf(global->fperr, "program global->lines %d and %d not in order\n",
		  global->lines[i-1].no, global->lines[i].no);
	  free(global->lines);
	  return -1;
	}
  ...
  global->dimvariables = 0;
  global->ndimvariables = 0;
...
static void cleanup(void)
{
  ...
  for(i=0;i<global->nvariables;i++)
	if(global->variables[i].sval)
	  free(global->variables[i].sval);
  if(global->variables)
	  free(global->variables);
  global->variables = 0;
  global->nvariables = 0;

  for(i=0;i<global->ndimvariables;i++)
  {
    if(global->dimvariables[i].type == STRID)
	{
	  if(global->dimvariables[i].str)
	  {
		size = 1;
        for(ii=0;ii<global->dimvariables[i].ndims;ii++)
		  size *= global->dimvariables[i].dim[ii];
	    for(ii=0;ii<size;ii++)
		  if(global->dimvariables[i].str[ii])
		    free(global->dimvariables[i].str[ii]);
		free(global->dimvariables[i].str);
	  }
	}
	else
	  if(global->dimvariables[i].dval)
		free(global->dimvariables[i].dval);
  }

  if(global->dimvariables)
	free(global->dimvariables);
...
/*
  ...
  checks the global global->errorflag.
  writes to global->fperr.
...
static void reporterror(int lineno)
{
  ...
  switch(global->errorflag)
  {
    case ERR_CLEAR:
	  assert(0);
	  break;
	case ERR_SYNTAX:
	  fprintf(global->fperr, "Syntax error line %d\n", lineno);
	  break;
	case ERR_OUTOFMEMORY:
	  fprintf(global->fperr, "Out of memory line %d\n", lineno);
	  break;
	case ERR_IDTOOLONG:
	  fprintf(global->fperr, "Identifier too long line %d\n", lineno);
	  break;
	case ERR_NOSUCHVARIABLE:
	  fprintf(global->fperr, "No such variable line %d\n", lineno);
	  break;
	case ERR_BADSUBSCRIPT:
	  fprintf(global->fperr, "Bad subscript line %d\n", lineno);
	  break;
	case ERR_TOOMANYDIMS:
	  fprintf(global->fperr, "Too many dimensions line %d\n", lineno);
	  break;
	case ERR_TOOMANYINITS:
	  fprintf(global->fperr, "Too many initialisers line %d\n", lineno);
	  break;
	case ERR_BADTYPE:
	  fprintf(global->fperr, "Illegal type line %d\n", lineno);
	  break;
	case ERR_TOOMANYFORS:
	  fprintf(global->fperr, "Too many nested fors line %d\n", lineno);
	  break;
	case ERR_NONEXT:
	  fprintf(global->fperr, "For without matching next line %d\n", lineno);
	  break;
	case ERR_NOFOR:
	  fprintf(global->fperr, "Next without matching for line %d\n", lineno);
	  break;
	case ERR_DIVIDEBYZERO:
	  fprintf(global->fperr, "Divide by zero lne %d\n", lineno);
	  break;
	case ERR_NEGLOG:
	  fprintf(global->fperr, "Negative logarithm line %d\n", lineno);
	  break;
	case ERR_NEGSQRT:
	  fprintf(global->fperr, "Negative square root line %d\n", lineno);
	  break;
	case ERR_BADSINCOS:
	  fprintf(global->fperr, "Sine or cosine out of range line %d\n", lineno);
	  break;
	case ERR_EOF:
	  fprintf(global->fperr, "End of input file %d\n", lineno);
	  break;
	case ERR_ILLEGALOFFSET:
	  fprintf(global->fperr, "Illegal offset line %d\n", lineno);
	  break;
	case ERR_TYPEMISMATCH:
	  fprintf(global->fperr, "Type mismatch line %d\n", lineno);
	  break;
	case ERR_INPUTTOOLONG:
	  fprintf(global->fperr, "Input too long line %d\n", lineno);
	  break;
	case ERR_BADVALUE:
	  fprintf(global->fperr, "Bad value at line %d\n", lineno);
	  break;
	case ERR_NOTINT:
	  fprintf(global->fperr, "Not an integer at line %d\n", lineno);
	  break;
	default:
	  fprintf(global->fperr, "ERROR line %d\n", lineno);
	  break;
  }
}
...
static int findline(int no)
{
  ...
  low = 0;
  high = global->nlines-1;
  while(high > low + 1)
  {
    mid = (high + low)/2;
	if(global->lines[mid].no == no)
	  return mid;
	if(global->lines[mid].no > no)
	  high = mid;
	else
  ...
  if(global->lines[low].no == no)
	mid = low;
  else if(global->lines[high].no == no)
	mid = high;
  else
	mid = -1;
...
static void doprint(void)
{
  ...
  while(1)
  {
    if(isstring(global->token))
	{
      str = stringexpr();
	  if(str)
	  {
        fprintf(global->fpout, "%s", str);
        free(str);
	  }
	}
	else
	{
	  x = expr();
	  fprintf(global->fpout, "%g", x);
	}
	if(global->token == COMMA)
	{
	  fprintf(global->fpout, " ");
	  match(COMMA);
	}
	else
	  break;
  }
  if(global->token == SEMICOLON)
  {
	match(SEMICOLON);
    fflush(global->fpout);
  }
...
static void dodim(void)
{
  ...
  switch(global->token)
  {
    case DIMFLTID:
	case DIMSTRID:
      getid(global->string, name, &len);
	  match(global->token);
	  dims[ndims++] = expr();
	  while(global->token == COMMA)
	  {
	    match(COMMA);
	    dims[ndims++] = expr();
		if(ndims > 5)
  ...
  if(global->token == EQUALS)
  {
	...
	switch(dimvar->type)
	{
      case FLTID:
		i = 0;
	    dimvar->dval[i++] = expr();
		while(global->token == COMMA && i < size)
		{
		  match(COMMA);
		  dimvar->dval[i++] = expr();
		  if(global->errorflag)
...
static int dofor(void)
{
  ...
  *lv.dval = initval;
  ...
  {
	savestring = global->string;
    while((global->string = strchr(global->string, '\n')))
	{
      global->errorflag = 0;
	  global->token = gettoken(global->string);
	  match(VALUE);
	  ...
	  {
	    match(NEXT);
		if(global->token == FLTID || global->token == DIMFLTID)
		{
          getid(global->string, nextid, &len);
		  if(!strcmp(id, nextid))
		  {
			answer = getnextline(global->string);
			global->string = savestring;
			global->token = gettoken(global->string);
  ...
  {
	strcpy(global->forstack[global->nfors].id, id);
	global->forstack[global->nfors].nextline = getnextline(global->string);
	global->forstack[global->nfors].step = stepval;
	global->forstack[global->nfors].toval = toval;
	global->nfors++;
    return 0;
  }
...
static int donext(void)
{
  ...
  if(global->nfors)
  {
    ...
    *lv.dval += global->forstack[global->nfors-1].step;
	if( (global->forstack[global->nfors-1].step < 0 && *lv.dval < global->forstack[global->nfors-1].toval) ||
		(global->forstack[global->nfors-1].step > 0 && *lv.dval > global->forstack[global->nfors-1].toval) )
	{
	  global->nfors--;
	  return 0;
	}
	...
	{
      return global->forstack[global->nfors-1].nextline;
	}
  }
  else
  {
...
static void doinput(void)
{
  ...
  {
  case FLTID:
	while(fscanf(global->fpin, "%lf", lv.dval) != 1)
	{
	  fgetc(global->fpin);
	  if(feof(global->fpin))
	  {
  ...
  case STRID:
	...
	if( fgets(buff, sizeof(buff), global->fpin) == 0)
	{
	  seterror(ERR_EOF);
	  return;
	}
...
/*
  ...
  Notes: missing global->variables (but not out of range subscripts)
...
static void lvalue(LVALUE *lv)
{
  ...
  switch(global->token)
  {
	...
	case DIMFLTID:
	case DIMSTRID:
	  type = (global->token == DIMFLTID) ? FLTID : STRID;
	  getid(global->string, name, &len);
	  match(global->token);
...
/*
  boolean factor, consists of expression relop expression
    or global->string relop global->string, or ( boolexpr() )
*/
static int boolfactor(void)
{
  int answer;
...
static double factor(void)
{
  ...
  switch(global->token)
  {
    case OPAREN:
	  ...
	  break;
	case VALUE:
	  answer = getvalue(global->string, &len);
	  match(VALUE);
	  break;
	case MINUS:
	  match(MINUS);
...
/*
  get the value of a scalar variable from global->string
  matches FLTID
*/
static double variable(void)
{
...
/*
  get value of a dimensioned variable from global->string.
  matches DIMFLTID
*/
static double dimvariable(void)
{
...
static VARIABLE *findvariable(const char *id)
{
  ...
  for(i=0;i<global->nvariables;i++)
	if(!strcmp(global->variables[i].id, id))
	  return &global->variables[i];
  return 0;
}
...
static DIMVAR *finddimvar(const char *id)
{
  ...
  for(i=0;i<global->ndimvariables;i++)
	if(!strcmp(global->dimvariables[i].id, id))
	  return &global->dimvariables[i];
  return 0;
}
...
/*
  ...
  works for both global->string and real arrays.
...
static VARIABLE *addfloat(const char *id)
{
  ...
  vars = realloc(global->variables, (global->nvariables + 1) * sizeof(VARIABLE));
  if(vars)
  {
	global->variables = vars;
    strcpy(global->variables[global->nvariables].id, id);
	global->variables[global->nvariables].dval = 0;
	global->variables[global->nvariables].sval = 0;
	global->nvariables++;
	return &global->variables[global->nvariables-1];
  }
  else
	seterror(ERR_OUTOFMEMORY);
...
/*
  add a global->string variable to table.
...
static DIMVAR *adddimvar(const char *id)
{
  ...
  vars = realloc(global->dimvariables, (global->ndimvariables + 1) * sizeof(DIMVAR));
  if(vars)
  {
    global->dimvariables = vars;
	strcpy(global->dimvariables[global->ndimvariables].id, id);
	global->dimvariables[global->ndimvariables].dval = 0;
	global->dimvariables[global->ndimvariables].str = 0;
	global->dimvariables[global->ndimvariables].ndims = 0;
	global->dimvariables[global->ndimvariables].type = strchr(id, '$') ? STRID : FLTID;
	global->ndimvariables++;
	return &global->dimvariables[global->ndimvariables-1];
  }
  else
	seterror(ERR_OUTOFMEMORY);
...
/*
  high level global->string parsing function.
...
/*
  parse the global->string$ global->token
*/
static char *stringstring(void)
{
  int x;
...
/*
  read a dimensioned global->string variable from input.
  Returns: pointer to global->string (not malloced)
*/
static char *stringdimvar(void)
{
  char id[32];
...
/*
  parse a global->string variable.
  Returns: pointer to global->string (not malloced)
*/
static char *stringvar(void)
{
...
/*
  parse a global->string literal
  Returns: malloced global->string literal
...
static char *stringliteral(void)
{
  ...
  while(global->token == QUOTE)
  {
    while(isspace(*global->string))
	  global->string++;

    end = mystrend(global->string, '"');
    if(end)
	{
      len = end - global->string;
      substr = malloc(len);
	  if(!substr)
	  {
	    seterror(ERR_OUTOFMEMORY);
	  ...
	  mystrgrablit(substr, global->string);
	  if(answer)
	  {
...
/*
  check that we have a global->token of the passed type
  (if not set the global->errorflag)
  Move parser on to next global->token. Sets global->token and global->string.
*/
static void match(int tok)
{
  if(global->token != tok)
  ...
  global->string += tokenlen(global->string, global->token);
  global->token = gettoken(global->string);
  if(global->token == ERROR)
	seterror(ERR_SYNTAX);
}

/*
  set the global->errorflag.
  Params: errorcode - the error.
  Notes: ignores error cascades
*/
static void seterror(int errorcode)
{
  if(global->errorflag == 0 || errorcode == 0)
	global->errorflag = errorcode;
}

/*
  get the next line number
  Params: str - pointer to parse global->string
...
/*
  get a global->token from the global->string
  Params: str - global->string to read global->token from
  ...
  switch(*str)
  {
	...
	default:
      ...
      if(!strncmp(str, "MID$", 4))
		return MIDSTRING;
      if(!strncmp(str, "global->string$", 7))
		return STRINGSTRING;
	  }
	  /* end isupper() */
...
/*
  get the length of a global->token.
  Params: str - pointer to the global->string containing the global->token
          global->token - the type of the global->token read
  Returns: length of the global->token, or 0 for EOL to prevent
           it being read past.
*/
static int tokenlen(const char *str, int token)
{
...
/*
  test if a global->token represents a global->string expression
  Params: global->token - global->token to test
  Returns: 1 if a global->string, else 0
*/
static int isstring(int token)
{
...
/*
  get a numerical value from the parse global->string
  Params: str - the global->string to search
          len - return pinter for no chars read
  Retuns: the value of the global->string.
*/
static double getvalue(const char *str, int *len)
{
  double answer;
...
/*
  getid - get an id from the parse global->string:
  Params: str - global->string to search
...
/*
  grab a literal from the parse global->string.
  Params: dest - destination global->string
          src - source global->string
...
/*
  find where a source global->string literal ends
  Params: src - global->string to check (must point to quote)
          quote - character to use for quotation
  Returns: pointer to quote which ends global->string
...
/*
  ...
  Params: str - global->string to check
...
/*
  duplicate a global->string:
  Params: str - global->string to duplicate
...
/*
  ...
  Params: str - firsts global->string
          cat - second global->string
  Returns: malloced global->string.
*/
static char *mystrconcat(const char *str, const char *cat)
{
  int len;
